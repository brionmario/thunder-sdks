...
/// Thunder SDK client — Platform layer implementation of the full IAMClient interface (spec §7.1).
public final class ThunderClient {
    private var config: ThunderConfig?
    ...
    public func reInitialize(baseUrl: String? = nil, clientId: String? = nil) async throws -> Bool {
        ...
        if let baseUrl { current = ThunderConfig(baseUrl: baseUrl, clientId: current.clientId) }
        config = nil
    ...
    @discardableResult
    private func requireInitialized() throws -> ThunderConfig {
    ...
    private func requireConfig() throws -> ThunderConfig {
        guard let config else {
            throw IAMError(code: .sdkNotInitialized, message: "Call initialize() before using the SDK")
        }
        return config
    ...
    private func validateConfig(_ config: ThunderConfig) throws {
